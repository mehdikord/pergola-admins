...
import Items_Create from "@/views/items/Items_Create.vue";
import Items_Edit from "@/views/items/Items_Edit.vue";
...
export default {
  ...
  methods :{
    Items_Get(per_page,page){
      ...
      Stores_Items().Index(this.query_params).then(res=>{
    ...
    Item_Delete(id){
      ...
      Stores_Items().Delete(id).then(res => {
    ...
    Item_Activation(id){
      ...
      Stores_Items().Activation(id).then(res => {
        this.items = this.items.filter(item => {
          if (item.id === id){
